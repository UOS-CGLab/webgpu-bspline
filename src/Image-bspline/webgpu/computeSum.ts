...
export default async function computeSum(
    ...
    const trianglePointNum: u32 = ${triangleUv.length / 2}; // 원의 점의 총 개수
    const blendLen: u32 = trianglePointNum * controlPointLen;
    ...
    @group(0) @binding(1) var<storage, read> triangleUV: array<vec2f, trianglePointNum>;
    @group(0) @binding(2) var<storage, read> blendResult: array<vec2f, trianglePointNum * controlPointLen>;
    @group(0) @binding(3) var<storage, read_write> pointResult: array<vec2f, trianglePointNum>;
    ...
    fn main (
      ...
      let pointIdx: u32 = global_invocation_id.x;
      var cp: vec2f = vec2f(0.0, 0.0);

      let u: u32 = u32(floor(triangleUV[pointIdx].x));
      let v: u32 = u32(floor(triangleUV[pointIdx].y));
	...
	const uvBuffer = device.createBuffer({
		size: triangleUv.length / 2 * 2 * 4,

		usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC,
		mappedAtCreation: true,
	});
	new Float32Array(uvBuffer.getMappedRange()).set(triangleUv);
	uvBuffer.unmap();

	const blendBuffer = device.createBuffer({
		size: triangleUv.length / 2 * ctrl.len * 2 * 4,
	...
	const dispatchCount = Math.ceil((triangleUv.length / 2) / workgroupSize);
	pass.dispatchWorkgroups(dispatchCount, 1, 1);
	// pass.dispatchWorkgroups(triangleUv.length / 2);
	pass.end();
