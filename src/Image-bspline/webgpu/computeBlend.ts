import {ctrl, bspline} from '../config';
import {calcKnotVector} from './arrayCalculation';

export default async function computeBlend(device: GPUDevice, triangleUv: Float32Array) {
    ...
    const circlePointNum: u32 = ${triangleUv.length / 2}; // 원의 점의 총 개수
    const knotVector: array<f32, knotVectorLen> = array<f32, knotVectorLen>(${knotVectorString});
    const blendLen: u32 = circlePointNum * controlPointLen;
    
    // 동적 배열 선언이 안 되기에 필요한 입력 값들을 shader 외부에서 생성한 다음에 binding group으로 보내줌
    @group(0) @binding(0) var<storage, read> circleUV: array<vec2f, circlePointNum>;
    @group(0) @binding(1) var<storage, read_write> blendResult: array<vec2f, blendLen>;
    ...
    */
	...
	const blendBindGroup = device.createBindGroup({
		...
		entries: [
			{binding: 0, resource: {buffer: uvBuffer}},
			{binding: 1, resource: {buffer: blendBuffer}},
		],
	});
